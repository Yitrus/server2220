...
//	return;
	...
	//add by yxy 基本页面的热阈值前移，不要那些数量最多的
	...
	for (tmp_index = 0; tmp_index < idx_hot; tmp_index++) {
		unsigned long nr_pages = memcg->hotness_hg[tmp_index];
		sum_pages += nr_pages;
		if(sum_pages > 100000){
			break;
		}
    }
	if(tmp_index+1 >= idx_hot){
		memcg->warm_threshold = memcg->active_threshold/5+1;
	}else{
		memcg->warm_threshold = tmp_index;
	}
}
