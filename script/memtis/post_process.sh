...
perf script -f --comms=train -Ftid,time,weight,event,phys_addr | sed 's/cpu\/mem-loads,ldlat=30\/P:/loads/g' | sed 's/cpu\/mem-stores\/P:/stores/g' | grep -w "loads" | awk '{OFS=","}{print $1,$4,"0x"$5}' | tr -d ":" > tid_weight_phys_addr.txt
...
perf script -f --comms=train | sed 's/cpu\/mem-loads,ldlat=30\/P:/loads/g' | sed 's/cpu\/mem-stores\/P:/stores/g' | grep -w "loads" | sed 's/TLB L1 or L2 hit/TLB_hit/g' | sed 's/TLB L2 miss/TLB_miss/g' | awk -F'|' '{print $3}' > tlb.txt
...
mv loads.txt "memory_trace_liblinear.csv"
