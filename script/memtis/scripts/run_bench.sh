...
CGROUP_NAME="htmm"
###### update DIR!
DIR=/home/xmu/playground/yi/script/memtis
NODE0=14GB
...
function func_collaction(){
	sleep 10
	# perf mem -D --phys-data --data record --all-user
	# perf mem -D --phys-data --data record --all-user
	# damo record $(pidof liblinear) 
	# damo record $(pidof XSBench) 
    # PID=$(pgrep -o train)
    # echo "---------Collaction ${PID}------------"
	# perf record --data --phys-data --data-page-size --pid=${PID} --exclude-perf -e ldlat-loads,ldlat-stores(mem-loads,mem-stores)
    # perf mem -D --phys-data --data record -k CLOCK_MONOTONIC --all-user  & 
    # sleep 10
	PID=$(pgrep -o XSBench)
	# perf mem -D --phys-data --data record --all-user --data-page-size &
...
function func_prepare() {
	...
	echo 1 > /sys/kernel/tracing/events/migrate/mm_migrate_page_user_vaddr_each/enable
	# echo 1 > /sys/kernel/tracing/events/tlb/migrate_page_each_file/enable
...
function func_main() {
    ...
    mkdir -p ${DIR}/results24/${BENCH_NAME}_${VER}
    LOG_DIR=${DIR}/results24/${BENCH_NAME}_${VER}

    # set memcg for htmm
    sudo ${DIR}/scripts/set_htmm_memcg.sh htmm remove
    sudo ${DIR}/scripts/set_htmm_memcg.sh htmm $$ enable
    sudo ${DIR}/scripts/set_mem_size.sh htmm 0 ${NODE0}
    echo "set cgroup sleep 8 sec"
    sleep 8
	...
	cat /sys/kernel/tracing/trace >> ${LOG_DIR}/trace.txt
    # disable htmm
